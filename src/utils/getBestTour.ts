...
export function getBest(routes: TourRoute[], maxScu?: number): TourRoute[] {
    ...
    for (let i = 0; i < routes.length; i++) {
      ...
      for (let j = 0; j < routes.length; j++) {
        ...
        const originMatch = routeA.origin.every((o) => routeB.origin.includes(o)) && routeA.origin.length === routeB.origin.length;
        const destinationMatch = routeA.destination.every((d) => routeB.destination.includes(d)) && routeA.destination.length === routeB.destination.length;
